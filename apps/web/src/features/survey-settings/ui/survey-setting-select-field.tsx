...
import type { SurveyRuntimeSettingOption } from '../survey-settings.types';
import { formatSurveyRuntimeApplyModeLabel } from '../survey-settings.types';

interface SurveySettingSelectFieldProps {
  id: string;
  label: string;
  description: string;
  value: string;
  onValueChange?: (value: string) => void;
  allowedValues: SurveyRuntimeSettingOption['allowedValues'];
  applyMode: SurveyRuntimeSettingOption['applyMode'];
...
export function SurveySettingSelectField({
  id,
  label,
  description,
  value,
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-white/80 p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <span className="rounded-full border border-[var(--border)] bg-[var(--panel-muted)] px-2.5 py-1 text-[11px] font-medium text-[var(--muted)]">
          {formatSurveyRuntimeApplyModeLabel(applyMode)}
        </span>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs">
        ...
        <span className="rounded-full bg-teal-50 px-2.5 py-1 font-medium text-teal-800">
          {isReadOnly ? 'Visible por rol' : 'Cambio permitido'}
        </span>
        {readOnlyNote ? (
