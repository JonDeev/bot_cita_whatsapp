import { useEffect, useState, type FormEvent } from 'react';
import { toast } from 'sonner';
import { ApiError } from '../../../shared/http/api-client';
import { StateMessage } from '../../../shared/ui/state-messages';
import { ReminderSettingsSection } from './reminder-settings-section';
import { ReminderSettingSelectField } from './reminder-setting-select-field';
import {
  formatReminderRuntimeAllowedRolesLabel,
  type ReminderRuntimeSettingsUpdateRequest,
  type ReminderRuntimeSettings,
  type ReminderRuntimeSettingsOptions,
} from '../reminder-settings.types';
import { useUpdateReminderRuntimeSettingsMutation } from '../reminder-settings.hooks';

interface ReminderSettingsAdvancedFormProps {
  settings: ReminderRuntimeSettings;
  options: ReminderRuntimeSettingsOptions;
}

const ADVANCED_FIELD_KEYS = ['dispatchBatchSize', 'eligibilityLimit'] as const;
type AdvancedFormValues = {
  dispatchBatchSize: ReminderRuntimeSettings['stored']['dispatchBatchSize'];
  eligibilityLimit: ReminderRuntimeSettings['stored']['eligibilityLimit'];
};

export function ReminderSettingsAdvancedForm({
  settings,
  options,
}: ReminderSettingsAdvancedFormProps) {
  const updateMutation = useUpdateReminderRuntimeSettingsMutation();
  const [values, setValues] = useState<AdvancedFormValues>({
    dispatchBatchSize: settings.stored.dispatchBatchSize,
    eligibilityLimit: settings.stored.eligibilityLimit,
  });
  const [feedback, setFeedback] = useState<{
    tone: 'neutral' | 'danger' | 'success';
    title: string;
    description?: string;
  } | null>(null);

  useEffect(() => {
    setValues({
      dispatchBatchSize: settings.stored.dispatchBatchSize,
      eligibilityLimit: settings.stored.eligibilityLimit,
    });
    setFeedback(null);
  }, [
    settings.metadata.version,
    settings.stored.dispatchBatchSize,
    settings.stored.eligibilityLimit,
  ]);

  const canEdit = settings.permissions.canEditAdvanced;
  const advancedFields = options.sections.advanced.filter((field) =>
    ADVANCED_FIELD_KEYS.includes(field.key as (typeof ADVANCED_FIELD_KEYS)[number]),
  );

  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setFeedback(null);

    const payload: ReminderRuntimeSettingsUpdateRequest = {
      expectedVersion: settings.metadata.version,
      changes: {
        dispatchBatchSize: values.dispatchBatchSize,
        eligibilityLimit: values.eligibilityLimit,
      },
    };

    try {
      const updated = await updateMutation.mutateAsync(payload);
      setFeedback({
        tone: 'success',
        title: 'Configuracion avanzada guardada.',
        description: `Version actualizada a #${updated.metadata.version}.`,
      });
      toast.success('Configuracion avanzada guardada.');
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) {
        setFeedback({
          tone: 'danger',
          title: 'Conflicto de version detectado.',
          description: 'Refresca la vista y vuelve a intentar el guardado.',
        });
        toast.warning('Conflicto de version. Refresca y vuelve a intentar.');
        return;
      }

      if (error instanceof ApiError) {
        setFeedback({
          tone: 'danger',
          title: 'No fue posible guardar la configuracion avanzada.',
          description: error.message,
        });
        toast.error(error.message);
        return;
      }

      setFeedback({
        tone: 'danger',
        title: 'No fue posible guardar la configuracion avanzada.',
        description: 'Intenta nuevamente en unos segundos.',
      });
      toast.error('No fue posible guardar la configuracion avanzada.');
    }
  };

  return (
    <ReminderSettingsSection
      title="Configuracion avanzada"
      description="Controles de cadencia y volumen para operacion tecnica."
      summary="Estos campos ajustan el ritmo del despachador."
      collapsible
      defaultOpen={false}
    >
      {!canEdit ? (
        <StateMessage
          title="Solo ADMIN puede modificar esta seccion."
          description="La vista se mantiene disponible para lectura y revision operativa."
          tone="warning"
        />
      ) : null}

      <form className="space-y-4" onSubmit={onSubmit}>
        {advancedFields.map((field) => {
          const fieldKey = field.key as (typeof ADVANCED_FIELD_KEYS)[number];

          return (
            <ReminderSettingSelectField
              key={fieldKey}
              id={`reminder-setting-${fieldKey}`}
              label={field.label}
              description={field.description}
              value={values[fieldKey]}
              onValueChange={(nextValue) =>
                setValues((current) => ({ ...current, [fieldKey]: nextValue }))
              }
              allowedValues={field.allowedValues}
              applyMode={field.applyMode}
              warningText={field.warningText}
              disabled={!canEdit || updateMutation.isPending}
              readOnlyNote={formatReminderRuntimeAllowedRolesLabel(field.editableByRoles)}
              helperText={
                fieldKey === 'dispatchBatchSize'
                  ? 'Controla cuantas operaciones procesa cada ciclo.'
                  : 'Acota cuantas citas elegibles se consultan en cada barrido.'
              }
            />
          );
        })}

        <div className="grid gap-3 rounded-2xl border border-[var(--border)] bg-[var(--panel-muted)] p-4 sm:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-[var(--muted)]">Guardado</p>
            <p className="mt-1 text-sm font-medium text-[var(--text)]">
              {settings.stored.dispatchBatchSize} / {settings.stored.eligibilityLimit}
            </p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-[var(--muted)]">Aplicacion</p>
            <p className="mt-1 text-sm font-medium text-[var(--text)]">
              {settings.runtimeApplication.restartScopedFieldKeys.length > 0
                ? 'Requiere reinicio controlado'
                : 'Aplicacion inmediata'}
            </p>
          </div>
        </div>

        {feedback ? (
          <StateMessage
            title={feedback.title}
            description={feedback.description}
            tone={feedback.tone}
          />
        ) : null}

        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs text-[var(--muted)]">
            El cambio queda versionado y auditado.
          </p>
          <button
            type="submit"
            disabled={!canEdit || updateMutation.isPending}
            className="inline-flex items-center justify-center rounded-xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {updateMutation.isPending ? 'Guardando...' : 'Guardar configuracion avanzada'}
          </button>
        </div>
      </form>
    </ReminderSettingsSection>
  );
}
